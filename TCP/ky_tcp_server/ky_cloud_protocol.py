...
class Data:
    ...
    def get_cmd_one(self) -> dict:
        return {'cmd_one': self.cmd_one}
    ...
    def get_all_data(self):
        ...
        if bool(self.cmd_one):
            reg_date_info = self.cmd_one.get('from_data_time_info')
            ai_pems_info = self.cmd_one.get('ai_pems_info')
            ai_pems_data = self.cmd_one.get('ai_pems_data')
            power_meter_data = self.cmd_one.get('power_meter_data')
            accumulated_energy_data = self.cmd_one.get('accumulated_energy_data')
            flow_meter_data = self.cmd_one.get('flow_meter_data')
            accumulated_flow_data = self.cmd_one.get('accumulated_flow_data')
            operating_time_data = self.cmd_one.get('operating_time_data')
            humidity_sensor_data = self.cmd_one.get('humidity_sensor_data')
            ...
            params['cmd_one'] = {
                ...
                "pressure": float(ai_pems_data['pressure']),
                "temperature": float(ai_pems_data['temperature']),
                "hz": float(ai_pems_data.get('hz', 0)),
                ...
                "average_watt": power_meter_data.get('apparent_power', 0),
                "instantaneous_e": power_meter_data.get('accumulated_energy', 0),
                "accumulate_e": accumulated_energy_data.get('monthly_energy', 0),
                ...
                "press_f": flow_meter_data.get('pressure', 0),
                "temperature_f": flow_meter_data.get('temperature', 0),
                "instantaneous_f": flow_meter_data.get('instantaneous_flow', 0),
                "accumulate_f": flow_meter_data.get('accumulated_flow', 0),
        ...
        if bool(self.cmd_three):
            cmd_three_list = list()
            for cmd_three_data in self.cmd_three:
                # Extract relevant information
                reg_date_info = cmd_three_data.get('from_data_time_info')
                ai_pems_info = cmd_three_data.get('ai_pems_info')

                info_type = cmd_three_data.get('info_type')
                arlam_info = cmd_three_data.get('arlam_info')
                # Construct datetime object from received information
                reg_date = datetime(reg_date_info['year'], reg_date_info['month'], reg_date_info['day'],
                                    reg_date_info['hour'], reg_date_info['minute'], reg_date_info['second'])
                # reg_date = reg_date.strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]
                cmd_three_list.append({
                    ...
                    "info_type": info_type,
                    # F: 고장정보, A: 경보정보 You need to provide the appropriate value for this parameter
                    "arlam_info": arlam_info  # 고장정보
                })
            params['cmd_three'] = cmd_three_list
    ...
    #             "equip_type": ai_pems_info['device_type'],
    #             "equip_no": ai_pems_info['device_sequence'],
    #             "pressure": ai_pems_data['pressure'],
    #             "temperature": ai_pems_data['temperature'],
    #             "hz": ai_pems_data.get('hz', 0),
    #             "setting_time": ai_pems_data.get('consumable_setup_time', 0),
    #             "use_time": ai_pems_data.get('consumable_usage_time', 0),
    #             "grease_setting_time": ai_pems_data.get('motor_grease_setup_time', 0),
    #             "grease_use_time": ai_pems_data.get('motor_grease_time', 0),
    #             "vsd_fsd": ai_pems_data.get('vsd_fsd', 0),  # 0: 정지, 1: 운전
    #             "driving_stop": ai_pems_data.get('operation_status', '') == '정지',
    #             # Assuming '운전' means '운전' and everything else is '정지'
    #             "total_driving": ai_pems_data.get('total_operation_time', 0),
    #             "average_voltage": power_meter_data.get('average_voltage', 0),
    #             "average_watt": power_meter_data.get('apparent_power', 0),
    ...
    #             "month_current_f": accumulated_flow_data.get('monthly_energy', 0),
    #             "year_current_f": accumulated_flow_data.get('yearly_energy', 0),
    #             "sun_operationtime": operating_time_data.get('sunday_energy', 0),
    #             "mon_operationtime": operating_time_data.get('monday_energy', 0),
    #             "tue_operationtime": operating_time_data.get('tuesday_energy', 0),
    #             "wed_operationtime": operating_time_data.get('wednesday_energy', 0),
    #             "thu_operationtime": operating_time_data.get('thursday_energy', 0),
    #             "fri_operationtime": operating_time_data.get('friday_energy', 0),
    #             "sat_operationtime": operating_time_data.get('saturday_energy', 0),
    #             "month_operationtime": operating_time_data.get('monthly_energy', 0),
    #             "year_operationtime": operating_time_data.get('yearly_energy', 0),
    #             "humidity": humidity_sensor_data.get('humidity', 0),
    #             "temperature_h": humidity_sensor_data.get('temperature', 0)
    #         }
    #
    #     if bool(self.cmd_three):
    #         # Extract relevant information
    #         reg_date_info = self.cmd_three.get('from_data_time_info')
    #         ai_pems_info = self.cmd_three.get('ai_pems_info')
    #
    #         # Construct datetime object from received information
    #         reg_date = datetime(reg_date_info['year'], reg_date_info['month'], reg_date_info['day'],
    #                             reg_date_info['hour'], reg_date_info['minute'], reg_date_info['second'])
    ...
    #             "equip_type": ai_pems_info['device_type'],
    #             "equip_no": ai_pems_info['device_sequence'],
    #             "info_type": "",  # You need to provide the appropriate value for this parameter
    #             "arlam_info": ""  # You need to provide the appropriate value for this parameter
    #         }
    #
    #     return params
...
def cmd_one(data):
    ...
    try:
        ...
        power_meter_data = {
            ...
            "apparent_power": struct.unpack('>f', power_meter_data_low_data[8:12])[0],
        ...
        accumulated_energy_data = decode_data_splite_day(accumulated_energy_data_low_data)
        ...
        accumulated_flow_data = decode_data_splite_day(accumulated_flow_data_low_data)
        operating_time_data = decode_data_splite_day(operating_time_data_low_data)

        humidity_sensor_data = {
            "humidity": int.from_bytes(humidity_sensor_data_low_data[:2], byteorder='big') / 10.0,
...
def cmd_three(data):
    ...
    try:
        ...
        if fault_data != 0:
            ...
            print(f"Hexadecimal representation of target_value: {formatted_target_hex}")
